 ...
 * It supports different algorithms for hashing and HMAC, and offers utilities for managing challenge options,
...
public class Altcha {
    ...
    public static class ChallengeOptions {
        ...
        public ChallengeOptions setUseSecureRandom(boolean useSecureRandom) {
     ...
     */
    public static byte[] randomBytes(int length, boolean useSecureRandom) {
     ...
     */
    public static int randomInt(long max, boolean useSecureRandom) {
        Random random = useSecureRandom ? new SecureRandom() : ThreadLocalRandom.current();
        return random.nextInt((int) max);
    }

    /**
     * Computes the hash of the given data using the specified algorithm, returning the result as a hex string.
    ...
    /**
     * Computes the HMAC of the given data using the specified algorithm and key, returning the result as a hex string.
    ...
    public static Challenge createChallenge(ChallengeOptions options) throws Exception {
        ...
        String salt = options.salt != null ? options.salt : bytesToHex(randomBytes((int) saltLength, options.useSecureRandom));
        if (!params.isEmpty()) {
            salt += "?" + encodeParams(params);
        }

        long number = options.number != null ? options.number : randomInt(maxNumber, false);
     ...
     * @throws Exception If an error occurs during verification.
     */
    public static ServerSignatureVerification verifyServerSignature(ServerSignaturePayload payload, String hmacKey) throws Exception {
     ...
     * @throws Exception If an error occurs during verification.
     */
    public static ServerSignatureVerification verifyServerSignature(String base64Payload, String hmacKey) throws Exception {
        String decodedPayload = new String(Base64.getDecoder().decode(base64Payload), StandardCharsets.UTF_8);
        JSONObject jsonObject = new JSONObject(decodedPayload);
    ...
    private static ServerSignatureVerification verifyServerSignatureInternal(ServerSignaturePayload payload, String hmacKey)
            throws Exception {
